...
class DataProcessor:
    def __init__(self):
        ...
        # Data containers
        self.waffle_demand = None  # Demand for each waffle type by week
        self.pan_supply = None     # Supply of each pan type by week
        self.waffle_cost = None    # Cost per waffle for each waffle-pan combination
        self.waffles_per_pan = None  # Number of waffles per pan for each pan type
        self.allowed_combinations = None  # Allowed combinations of waffle and pan types
        
        # Processed data
        self.waffle_types = set()  # Set of waffle types
        self.pan_types = set()     # Set of pan types
        self.weeks = set()         # Set of weeks
    ...
    def get_feasibility_data(self) -> Dict:
        ...
        total_demand = {}
        for week in self.weeks:
            total_demand[week] = sum(self.demand_dict.get((wt, week), 0) for wt in self.waffle_types)
            
        total_supply = {}
        for week in self.weeks:
            total_supply[week] = sum(self.supply_dict.get((pt, week), 0) for pt in self.pan_types)
