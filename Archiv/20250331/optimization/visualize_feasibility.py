...
class FeasibilityVisualizer:
    ...
    def _plot_supply_vs_demand(self, fig, position):
        """Plot weekly supply vs demand as bar chart with improved visual design."""
        ax = fig.add_subplot(position)
        
        # Get all weeks in sorted order
        weeks = sorted(self.checker.data['weeks'])
        
        # Get weekly supply and demand data
        weekly_supply = {}
        weekly_demand = {}
        
        for week in weeks:
            weekly_supply[week] = self.checker.data['total_supply'].get(week, 0)
            weekly_demand[week] = self.checker.data['total_demand'].get(week, 0)
        
        # Create x-positions for the bars
        x = np.arange(len(weeks))
        width = 0.35
        
        # Create color schemes based on supply vs demand relationship
        supply_colors = []
        demand_colors = []
        warning_symbols = []
        
        for week in weeks:
            supply = weekly_supply[week]
            demand = weekly_demand[week]
            
            if supply >= demand * 1.1:  # Good - 10% buffer
                supply_colors.append('#2E8B57')  # Sea Green
                demand_colors.append('#1E90FF')  # Dodger Blue
                warning_symbols.append('')
            elif supply >= demand:  # Adequate but tight
                supply_colors.append('#FFD700')  # Gold
                demand_colors.append('#1E90FF')  # Dodger Blue
                warning_symbols.append('⚠')
            else:  # Insufficient
                supply_colors.append('#FF6347')  # Tomato
                demand_colors.append('#1E90FF')  # Dodger Blue
                warning_symbols.append('✗')
        
        # Plot the bars
        supply_bars = ax.bar(x - width/2, [weekly_supply[w] for w in weeks], width, 
                             color=supply_colors, label='Supply', edgecolor='black', linewidth=0.5)
        demand_bars = ax.bar(x + width/2, [weekly_demand[w] for w in weeks], width, 
                             color=demand_colors, label='Demand', edgecolor='black', linewidth=0.5)
        
        # Add warning symbols where needed
        for i, symbol in enumerate(warning_symbols):
            if symbol:
                ax.annotate(symbol, 
                           xy=(x[i], max(weekly_supply[weeks[i]], weekly_demand[weeks[i]]) * 1.05),
                           fontsize=14, 
                           color='red' if symbol == '✗' else 'orange',
                           ha='center', va='bottom')
        
        # Customize plot
        ax.set_xlabel('Week', fontweight='bold')
        ax.set_ylabel('Number of Pans', fontweight='bold')
        ax.set_title('Weekly Supply vs. Demand', fontsize=12, fontweight='bold')
        ax.set_xticks(x)
        ax.set_xticklabels(weeks)
        
        # Add total supply vs demand as a text box
        total_supply = sum(weekly_supply.values())
        total_demand = sum(weekly_demand.values())
        buffer_percentage = (total_supply / total_demand * 100) if total_demand > 0 else float('inf')
        
        buffer_text = f"Total Supply: {total_supply:,.0f}\nTotal Demand: {total_demand:,.0f}\nBuffer: {buffer_percentage:.1f}%"
        buffer_color = 'green' if buffer_percentage >= 110 else ('orange' if buffer_percentage >= 100 else 'red')
        
        # Add a background box with the summary
        props = dict(boxstyle='round', facecolor='white', alpha=0.7, edgecolor=buffer_color, linewidth=2)
        ax.text(0.05, 0.95, buffer_text, transform=ax.transAxes, fontsize=10,
                verticalalignment='top', bbox=props)
        
        # Add data labels on bars for visibility
        def add_labels(bars):
            for bar in bars:
                height = bar.get_height()
                if height > 0:  # Only add labels to bars with height > 0
        ...
        # Add legend with better position and style
        ax.legend(loc='upper right', frameon=True, fancybox=True, framealpha=0.7, 
        ...
        # Add grid for the y-axis only with light gray color
