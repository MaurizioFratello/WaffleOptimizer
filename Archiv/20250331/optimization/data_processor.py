...
class DataProcessor:
    ...
    def get_feasibility_data(self) -> Dict:
        ...
        print(f"Debug: Available weeks: {self.weeks}")
        print(f"Debug: Available waffle types: {self.waffle_types}")
        print(f"Debug: Available pan types: {self.pan_types}")
        ...
        for week in self.weeks:
            ...
            print(f"Debug: Week {week} demand: {week_demand}")
            print(f"Debug: Demand dict entries for week {week}:")
            for wt in self.waffle_types:
                print(f"  {wt}: {self.demand_dict.get((wt, week), 0)}")
            
        total_supply = {}
        for week in self.weeks:
            week_supply = sum(self.supply_dict.get((pt, week), 0) for pt in self.pan_types)
            total_supply[week] = week_supply
            print(f"Debug: Week {week} supply: {week_supply}")
            print(f"Debug: Supply dict entries for week {week}:")
            for pt in self.pan_types:
                print(f"  {pt}: {self.supply_dict.get((pt, week), 0)}")
        
        # Debug logging
        print(f"Debug: Calculated total_demand: {total_demand}")
        print(f"Debug: Calculated total_supply: {total_supply}")
        ...
        # Debug logging
        print(f"Debug: Final data dictionary keys: {list(data.keys())}")
        print(f"Debug: Data dictionary contents:")
