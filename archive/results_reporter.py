...
class ResultsReporter:
    ...
    def export_solution(self, filename: str) -> None:
        ...
        # Ensure proper sorting of Week, Waffle, and Pan columns
        # First, convert Week to numeric for proper sorting
        detailed_report['Week'] = pd.to_numeric(detailed_report['Week'].str.extract('(\d+)')[0], errors='coerce')
        
        # Sort the detailed report
        detailed_report = detailed_report.sort_values(['Week', 'WaffleType', 'PanType'])
        
        # Reorder columns to put Week, Waffle, Pan first
        column_order = ['Week', 'WaffleType', 'PanType', 'PansUsed', 'WafflesProduced', 'UnitCost', 'TotalCost']
        detailed_report = detailed_report[column_order]
